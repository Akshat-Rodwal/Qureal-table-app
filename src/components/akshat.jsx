...
import React, { useState, useEffect, useRef } from "react";
import { useReactTable, getCoreRowModel } from "@tanstack/react-table";
import styles from "./TableComponent.module.css"; // Assuming you're using CSS Modules

const TableComponent = ({ value, onChange }) => {
  // Table data and column configuration with localStorage or fallback to initial data.
  const [tableData, setTableData] = useState(() => {
    const savedData = sessionStorage.getItem("tableData");
    return savedData
      ? JSON.parse(savedData)
  ...
  // Handle cell styles (for text color and background color)
  const [cellStyles, setCellStyles] = useState(() => {
    const savedStyles = sessionStorage.getItem("cellStyles");
    return savedStyles ? JSON.parse(savedStyles) : {};
  });

  // Manage selected rows and columns
  const [showRowCheckboxes, setShowRowCheckboxes] = useState(false);
  const [showColumnCheckboxes, setShowColumnCheckboxes] = useState(false);
  const [selectedRows, setSelectedRows] = useState(new Set());
  const [selectedColumnsForRemoval, setSelectedColumnsForRemoval] = useState(
  ...
  // Handle table data update in sessionStorage
  useEffect(() => {
    sessionStorage.setItem("tableData", JSON.stringify(tableData));
    sessionStorage.setItem("tableColumns", JSON.stringify(tableColumns));
    sessionStorage.setItem("cellStyles", JSON.stringify(cellStyles));
  ...
  }, [tableData, tableColumns, cellStyles, onChange]);
      ...
      {selectedCell && (
        <div className={styles.controls}>
          <input
            type="text"
            value={
              tableData[selectedCell.rowIndex][selectedCell.columnId] || ""
            }
            onChange={(e) =>
              handleCellEdit(
                  ...
                  {headerGroup.headers.map((header) => (
                    <span key={header.id}>
                      <input
                        ...
                        onChange={(e) =>
                          handleColumnHeaderEdit(header.index, e.target.value)
                        }
                      />
                    </span>
                  ))}
...
// const TableComponent = ({ value, onChange }) => {
//   // Table data and column configuration with sessionStorage or fallback to initial data.
//   const [tableData, setTableData] = useState(() => {
//     const savedData = sessionStorage.getItem("tableData");
//     return savedData
//       ? JSON.parse(savedData)
